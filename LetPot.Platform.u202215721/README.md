...
- **Domain-Driven Design**: Clean architecture with bounded contexts (Telemetry and Allocation)
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=localhost;Database=let_pot;User=root;Password=your_password;"
  }
}
```
...
The application uses Entity Framework Core with automatic migrations. The database schema `let_pot` will be created automatically on first run with the following tables:
- `data_records` - Telemetry data from pots
